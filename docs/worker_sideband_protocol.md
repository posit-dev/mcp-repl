...
This document defines worker protocol version 9. The server rejects unsupported
...
That event means the worker is ready for the next `input_batch`.
`input_wait.prompt` is display data: a string prompts the client to present a
prompt, and `null` means prompt-free readiness.
...
Prompt strings are display/cache data. The server does not infer input routing,
completion, or interrupt behavior from prompt text, raw stdout, raw stderr, PTY
state, stdin writes, or timing.
...
- `{ "type": "discard_pending_input", "discard_id": <integer> }`
- Sent when the client requests interrupt.
- The server sends this cleanup message first, waits briefly for
  `discard_pending_input_ack` carrying the same `discard_id`, and then delivers
  a platform interrupt to the worker process or process group when a worker
...
- The IPC message carries no input and does not complete a batch. It tells the
  ...
  runtime.
- `discard_id` is assigned by the server and is scoped to the worker
  connection. Workers must echo it in the matching
  `discard_pending_input_ack`.
- Sending `discard_pending_input` does not change server-side readiness.
  Readiness changes only when the server sends `input_batch` or receives
  `input_wait`.
...
The server emits no other server-to-worker protocol messages in v9.
...
`worker_ready`
- `{ "type": "worker_ready", "protocol": { "name": "mcp-repl-worker", "version": 9 }, "worker": { "name": <string>, "version": <string> }, "capabilities": { "images": <bool> } }`
- Normal first worker message.
- `protocol.name` must be `mcp-repl-worker`, and `protocol.version` must be `9`.
...
`input_wait`
- `{ "type": "input_wait", "prompt": <string|null> }`
- Marks the worker ready for the next `input_batch`.
- `prompt: null` is prompt-free readiness. It suppresses prompt presentation and
  generated input echo for that boundary.
...
`discard_pending_input_ack`
- `{ "type": "discard_pending_input_ack", "discard_id": <integer>, "discarded_input": <bool> }`
- Confirms that the worker processed the ordered sideband
  `discard_pending_input` cleanup message with the same `discard_id`.
...
`input_line`
- `{ "type": "input_line", "prompt": <string|null>, "text": <string> }`
...
- `prompt: null` suppresses generated input echo for the delivered text.
- Submitted input must not be emitted as `output_text`.
- The server may reconstruct non-null `prompt + text` from ordered `input_line`
  events for transcript finalization, debugging views, or other surfaces that
  need an interactive transcript.
...
The worker emits no other worker-to-server protocol messages in v9.
...
worker queues the batch through its runtime input primitive. Sending to that
primitive is what wakes any blocked runtime input consumer. When the runtime
consumes all queued input and asks the managed input boundary for more, the
worker emits `input_wait`; a prompt-free top-level loop emits
`input_wait` with `prompt: null`.
...
`discard_pending_input`, and `shutdown`, mutates worker-owned queue/session
state, emits `discard_pending_input_ack` after cleanup, and never injects
runtime interrupts. Runtime interruption comes from the server's platform
interrupt delivery, not from the sideband cleanup message. The runtime thread
consumes queued input only when the runtime calls its managed input boundary. A
`shutdown` message asks built-in workers to stop accepting new input, preserve
already accepted input until it is consumed, wake managed input readers with EOF
after that queue drains, and exit after the active runtime request reaches a
safe boundary or the server's bounded shutdown window expires. The server still
applies the worker's process stdin shutdown policy so stdin-backed readers can
be unblocked without making stdin EOF the only shutdown signal.
...
non-empty tool call, the worker decides where the next `input_batch` is
consumed based on where the runtime is blocked. If the top-level cell loop is
blocked, the payload is one complete Python cell. If Python is blocked inside
`PyOS_Readline` or managed stdin, the payload is stdin for that reader.
`input_wait` with `prompt: null` is normal cell readiness, not a missing prompt.
Interrupt does not change readiness by itself. If a Ctrl-C input includes a
tail, the server sends the tail after the sideband cleanup/ack attempt,
...
`discard_pending_input` whenever the client asks for interrupt, and it must also
send the platform interrupt when a worker process exists. The sideband message
is the worker-owned cleanup signal; the platform interrupt is delivered to the
runtime or process group, and the runtime handles it according to its own rules.

When the worker receives sideband `discard_pending_input`, it must discard
managed input that is still queued and has not yet been consumed by the runtime.
The discard is triggered by the IPC message, not by the OS interrupt. The worker
must not emit `input_wait` again until it is actually ready for input.
...
The following older protocol concepts are not part of v9:

- input IDs on `input_batch`, `input_line`, `input_wait`, or `session_end`
- `ready`
