...
- Files-mode bundle notices are structured. Text overages start with `Oversized output`, disclose `Full output: ...`, optionally report shown line/character ranges and omitted quota tail content, then show labeled preview sections. Mixed text/image overages start with `Oversized mixed output`, report inline image anchors, and list `Ordered output index: ...`, `Transcript: ...`, and/or `Images: ...`. If a bundle cannot be created, the notice says `Output bundle unavailable`.
...
- Control: `\u0003` in input interrupts; `\u0004` (Ctrl-D / EOF) restarts the session. If input remains after `\u0004`, the same reply includes bounded-shutdown output followed by fresh-session output under the original call timeout.
