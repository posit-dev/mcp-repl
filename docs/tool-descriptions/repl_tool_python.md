...
- Control prefixes in `input`: `\u0003` (interrupt) and `\u0004` (Ctrl-D / EOF restart; if input remains after `\u0004`, the same response includes restart output plus output produced by the remaining input, bounded by the original call timeout).
