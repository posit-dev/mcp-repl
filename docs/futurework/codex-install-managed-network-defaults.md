# Codex Install Managed Network Defaults

## Motivation

After `mcp-repl install --client codex`, common package and documentation tasks
should work without requiring users to hand-edit Codex's `config.toml` first.
The default install should give the worker a narrow, managed network path to
trusted package and documentation sources while unrelated worker network access
remains blocked.

Minimal task:

1. A user runs `mcp-repl install --client codex`.
2. The installer updates Codex's `config.toml` with the normal R and Python MCP
   servers and a curated managed-network allowlist.
...
- Codex install is written by `src/install.rs`.
...
- Preserve `--sandbox inherit`. Do not add install defaults in a way that
  accidentally turns Codex installs into a fixed `workspace-write` sandbox.
...
- Decide whether defaults belong in Codex's own sandbox configuration, in
  `mcp-repl` server args, or in a new install-time helper that only affects
  workspace-write calls. Codex and `mcp-repl` should agree on the effective
  policy seen by the agent.
- Make the install idempotent. Re-running `mcp-repl install --client codex`
  should not duplicate allowlist entries or discard user-added entries.
...
- Add a failing install test that runs `mcp-repl install --client codex` in an
  isolated `CODEX_HOME` and asserts that the generated `config.toml` contains
  the default managed-network entries in the chosen config location.
- Assert that generated MCP server entries still preserve `--sandbox inherit`.
- Assert that read-only inherited sandbox metadata does not fail solely because
  install added managed-network defaults for workspace-write calls.
