...
- `scripts/public_api_suite.py`: external public API checks, including basic R `repl`, timeout/busy recovery, and `repl_reset` state clearing against an already-built `mcp-repl` binary over MCP stdio.
