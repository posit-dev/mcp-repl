...
draw_code <- function(x, y, label, cex) {
  ...
  height <- strheight(label, units = "user", cex = cex, font = 2, family = "mono")
  rect(
    x - width / 2 - 0.55,
    y - height / 2 - 0.42,
    x + width / 2 + 0.55,
...
draw_inline_code_label <- function(x, y, code, suffix, cex = 1.35) {
  ...
  code_width <- strwidth(code, units = "user", cex = cex, font = 2, family = "mono")
...
draw_mcp_repl_box <- function(x0, y0, x1, y1, fill, border, cex = 0.95) {
  ...
  text((x0 + x1) / 2, (y0 + y1) / 2 - 2.05, "R session", col = fg, cex = cex, font = 2)
}
...
text(75, 49.5, "Posit Assistant: human in the loop", col = fg, cex = 1.35, font = 2)
text(
  c(25, 75),
  c(13, 10.7),
  c(
