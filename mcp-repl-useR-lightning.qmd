...
::: {.columns}
::: {.column width="58%"}
![](mcp-repl-useR-lightning-assets/chat-rscript-e.png){fig-alt="A chat interface where a user asks an agent to explore a dataset. The agent responds by running a bash tool call named bash with an inline Rscript command." width="100%"}
:::
...
result. The chat feels persistent. But the execution surface underneath is
often a bash tool call. The agent starts with an inline Rscript command
because that is the shortest path to asking R one question.
:::
...
::: {.columns}
::: {.column width="58%"}
![](mcp-repl-useR-lightning-assets/chat-heredoc-script.png){fig-alt="A chat interface where the assistant uses a bash tool call named bash to write an R script with a heredoc, then runs it with Rscript." width="100%"}
:::
...
::: notes
The next step is predictable. Inline strings become too hard to quote and
too easy to break. So the agent writes a temporary script, usually with a
heredoc or a generated file, then calls Rscript on that file. This is more
stable than one giant command string, but it is still batch execution.
:::
...
::: {.columns}
::: {.column width="58%"}
![](mcp-repl-useR-lightning-assets/chat-rerun-loop.png){fig-alt="A chat interface where the assistant repeatedly runs the same Rscript file through a bash tool call named bash." width="100%"}
:::

::: {.column width="42%"}
Every attempt starts cold.

Run 1: load data -> inspect columns
Run 2: load data -> clean data -> plot
Run 3: load data -> clean data -> fit model
Run 4: load data -> clean data -> debug error
...
a repl tool for input and a reset tool for starting over. Most of the
