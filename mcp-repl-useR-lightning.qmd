...
draw_arrow_between <- function(
  ...
  shaft_fraction <- shaft_length / visual_distance
  trim_fraction <- (end_fraction - start_fraction - shaft_fraction) / 2
  stopifnot(trim_fraction >= 0)
  arrow_start <- start_fraction + trim_fraction
  arrow_end <- end_fraction - trim_fraction
  draw_arrow(
    x0 + arrow_start * dx,
    y0 + arrow_start * dy,
    x0 + arrow_end * dx,
...
agent_arrow <- list(shaft_length = 0.67, head_length = 0.10, lwd = 2.8)
ide_arrow <- list(shaft_length = 1.00, head_length = 0.11, lwd = 2.8)
...
triangle_side <- 19.5
triangle_top_y <- 41.5
triangle_left <- c(63, triangle_top_y)
triangle_right <- c(triangle_left[1] + triangle_side, triangle_top_y)
triangle_bottom <- c(
  mean(c(triangle_left[1], triangle_right[1])),
  triangle_top_y - triangle_side * x_unit / y_unit * sqrt(3) / 2
)
