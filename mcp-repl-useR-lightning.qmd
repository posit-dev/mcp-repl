...
<img alt="A chat interface where a user asks an agent to explore a
dataset and build a first model. The agent responds by running several
bash tool calls with Rscript and an edit_file tool call."
style="width: 100%; fig-align='center'"
src="mcp-repl-useR-lightning-assets/chat-scenario.png"
>
...
::: {.columns}
::: {.column width="58%"}
<img src="mcp-repl-useR-lightning-assets/chat-rscript-e.png"
alt="A chat interface where a user asks an agent to explore a dataset. The agent responds by running a bash tool call named bash with an inline Rscript command."
style="width: 100%;">
:::
...
::: {.columns}
::: {.column width="58%"}
<img src="mcp-repl-useR-lightning-assets/chat-rerun-loop.png" alt="A chat interface where the assistant runs an Rscript file, edits it with an edit_file tool call, and runs it again." style="width: 100%;">
:::
...
## R Is Rich in Interactive Surfaces
...
- These are designed for repeated interactions with a persistent
  stateful R session.
- Many require `interactive()` return `TRUE`.
...
review work, but the agent owns the live connection to `mcp-repl`, and
the session can also run unattended. There is no direct line from a
...
source, and it helps external agents use R more effectively than raw
