...
impl<R: ServiceRole> Service<R> for Box<dyn DynService<R>> {
    ...
    fn on_response_sent(
        ...
        DynService::on_response_sent(self.as_ref(), request_id)
    }
...
pub trait DynService<R: ServiceRole>: Send + Sync {
    ...
    fn on_response_sent(&self, _request_id: RequestId) -> BoxFuture<'_, ()> {
...
impl<R: ServiceRole, S: Service<R>> DynService<R> for S {
    ...
    fn on_response_sent(&self, request_id: RequestId) -> BoxFuture<'_, ()> {
        Box::pin(self.on_response_sent(request_id))
    }
    fn get_info(&self) -> R::Info {
        self.get_info()
    }
