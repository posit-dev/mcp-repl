...
fn worker_sideband_protocol_keeps_images_one_way() {
    ...
    for required in [
        ...
        r#"{ "type": "worker_ready", "protocol": { "name": "mcp-repl-worker", "version": 8 }, "worker": { "name": <string>, "version": <string> }, "capabilities": { "images": <bool> } }"#,
        r#"{ "type": "input_batch", "input": <string> }"#,
        r#"{ "type": "input_line", "prompt": <string>, "text": <string> }"#,
        r#"{ "type": "input_wait", "prompt": <string> }"#,
        r#"{ "type": "ready" }"#,
        r#"{ "type": "interrupt_ack", "interrupt_id": <integer>, "discarded_input": <bool> }"#,
        r#"{ "type": "session_end", "reason": <string>, "message": <string, optional> }"#,
        r#"{ "type": "interrupt", "interrupt_id": <integer> }"#,
        "This document defines worker protocol version 8.",
