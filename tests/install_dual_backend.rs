...
#[test]
fn install_claude_reinstall_preserves_unrelated_interpreter_server_hooks() -> TestResult<()> {
    let temp = tempfile::tempdir()?;
    let claude_dir = temp.path().join(".claude");
    std::fs::create_dir_all(&claude_dir)?;
    let config_path = temp.path().join(".claude.json");
    ...
    assert!(
        startup_commands.contains(&custom_session_start.as_str()),
        "expected unrelated interpreter-based SessionStart command to remain"
    );
    assert!(
        clear_commands.contains(&custom_session_end.as_str()),
        "expected unrelated interpreter-based SessionEnd command to remain"
    );
