...
#[tokio::test(flavor = "multi_thread")]
async fn python_detached_sys_stdin_partial_read_allows_fresh_cell_before_next_wait()
-> TestResult<()> {
    let _guard = lock_test_mutex();
    let Some(session) = start_python_session().await? else {
        return Ok(());
    ...
    let second_wait_path = temp_dir.path().join("second-text-wait-before-fresh");
    ...
    let second_wait_path_literal = serde_json::to_string(
        second_wait_path
            .to_str()
            .ok_or("second text wait path must be valid utf-8")?,
    )?;
...
second_wait_path = pathlib.Path({second_wait_path_literal})
...
def read_pair():
    ...
    while not release_path.exists():
        time.sleep(0.01)
    second_wait_path.write_text("waiting")
...
"#
    ...
    let fresh = session
        .write_stdin_raw_with("print('DETACHED_TEXT_STALE_FRESH_CELL')", Some(5.0))
        .await?;
    let fresh_text = result_text(&fresh);
    assert!(
        fresh_text.contains("DETACHED_TEXT_STALE_FRESH_CELL"),
        "expected fresh cell to run before detached sys.stdin asks for more input, got: {fresh_text:?}"
    );

    fs::write(&release_path, "go")?;
    text.push_str(&fresh_text);
    wait_for_file_text(&second_wait_path, "waiting").await?;
    ...
    assert!(
        text.contains("DETACHED_TEXT_STALE_PAIR xz"),
        "expected detached sys.stdin reader to consume fresh stdin instead of stale answer bytes, got: {text:?}"
    );
    assert!(
        !text.contains("DETACHED_TEXT_STALE_PAIR xy"),
        "detached sys.stdin reader consumed stale answer bytes: {text:?}"
    );
    Ok(())
}
...
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread")]
async fn python_detached_raw_stdin_partial_read_allows_fresh_cell_before_next_wait()
-> TestResult<()> {
    let _guard = lock_test_mutex();
    let Some(session) = start_python_session().await? else {
        return Ok(());
    ...
    let second_wait_path = temp_dir.path().join("second-raw-wait-before-fresh");
    ...
    let second_wait_path_literal = serde_json::to_string(
        second_wait_path
            .to_str()
            .ok_or("second raw wait path must be valid utf-8")?,
    )?;
    let release_path_literal = serde_json::to_string(
        release_path
            .to_str()
...
"#
    ...
    let fresh = session
        .write_stdin_raw_with("print('DETACHED_RAW_STALE_FRESH_CELL')", Some(5.0))
        .await?;
    let fresh_text = result_text(&fresh);
    assert!(
        fresh_text.contains("DETACHED_RAW_STALE_FRESH_CELL"),
        "expected fresh cell to run before detached raw stdin asks for more input, got: {fresh_text:?}"
    );
    ...
    assert!(
        text.contains("DETACHED_RAW_STALE_PAIR xz"),
        "expected detached raw stdin reader to consume fresh stdin instead of stale answer bytes, got: {text:?}"
    );
    assert!(
        !text.contains("DETACHED_RAW_STALE_PAIR xy"),
        "detached raw stdin reader consumed stale answer bytes: {text:?}"
    );
    Ok(())
}
...
#[tokio::test(flavor = "multi_thread")]
async fn python_background_input_does_not_detach_newly_accepted_cell() -> TestResult<()> {
    let _guard = lock_test_mutex();
    ...
    for attempt in 0..6 {
        ...
        let temp_dir = tempdir()?;
        let release_path = temp_dir.path().join("release-background-input");
        let release_path_literal = serde_json::to_string(
            release_path
                .to_str()
                .ok_or("release path must be valid utf-8")?,
        )?;
        let ready_marker = format!("RACE_BACKGROUND_READY_{attempt}");
        let foreground_marker = format!("RACE_FOREGROUND_CELL_{attempt}");

        let setup = session
            .write_stdin_raw_with(
                format!(
                    r#"
import _mcp_repl, pathlib, threading, time
release_path = pathlib.Path({release_path_literal})

def contender(index):
    while not release_path.exists():
        time.sleep(0)
    while not _mcp_repl.has_request_active():
        time.sleep(0)
    time.sleep(0)
    value = input(f"race-{attempt}-{{index}}> ")
    print("RACE_BACKGROUND_STOLE", {attempt}, index, value, flush=True)

for index in range(32):
    threading.Thread(target=contender, args=(index,), daemon=True).start()
...
"#
        ...
        let setup_text = result_text(&setup);
        assert!(
            setup_text.contains(&ready_marker),
            "attempt {attempt}: expected background contenders to start, got: {setup_text:?}"
        );

        fs::write(&release_path, "go")?;
        let foreground = session
            .write_stdin_raw_with(format!("print('{foreground_marker}')"), Some(5.0))
            .await?;
        let foreground_text = result_text(&foreground);
        assert!(
            foreground_text.contains(&foreground_marker),
            "attempt {attempt}: newly accepted cell was detached or stolen, got: {foreground_text:?}"
        );
        assert!(
            !foreground_text.contains("RACE_BACKGROUND_STOLE"),
            "attempt {attempt}: background input consumed the foreground cell request, got: {foreground_text:?}"
        );
        session.cancel().await?;
    }
...
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread")]
async fn python_competing_stdin_readers_do_not_steal_active_prompt_answer() -> TestResult<()> {
    let _guard = lock_test_mutex();
...
def owner():
    value = input("owner> ")
    print("OWNER_RESULT", value, flush=True)

def line_stealer(index):
    value = input(f"line-stealer-{index}> ")
    print("LINE_STEALER_RESULT", index, value, flush=True)

def raw_stealer(index):
    data = os.read(0, 64).decode("utf-8")
    print("RAW_STEALER_RESULT", index, data, flush=True)

threading.Thread(target=owner, daemon=True).start()
time.sleep(0.2)
for index in range(12):
    threading.Thread(target=line_stealer, args=(index,), daemon=True).start()
    threading.Thread(target=raw_stealer, args=(index,), daemon=True).start()
time.sleep(0.4)
print("COMPETING_READERS_READY", flush=True)
"#,
                Some(5.0),
            )
            .await?;
        ...
        assert!(
            !is_busy_response(&setup_text),
            "attempt {attempt}: expected owner prompt setup to complete, got: {setup_text:?}"
        );
        assert!(
            setup_text.contains("owner> "),
            "attempt {attempt}: expected owner prompt, got: {setup_text:?}"
        );

        sleep(Duration::from_millis(800)).await;

        let answer = format!("owner-answer-{attempt}");
        let result = session.write_stdin_raw_with(&answer, Some(2.0)).await?;
        let mut text = result_text(&result);
        let deadline = Instant::now() + Duration::from_secs(5);
        while Instant::now() < deadline
            && !text.contains(&format!("OWNER_RESULT {answer}"))
            && !text.contains("STEALER_RESULT")
        {
            sleep(Duration::from_millis(50)).await;
            let poll = session
                .write_stdin_raw_unterminated_with("", Some(1.0))
        ...
        assert!(
            text.contains(&format!("OWNER_RESULT {answer}")),
            "attempt {attempt}: active prompt owner did not consume answer, got: {text:?}"
        );
        assert!(
            !text.contains("LINE_STEALER_RESULT") && !text.contains("RAW_STEALER_RESULT"),
            "attempt {attempt}: non-owner stdin reader consumed answer, got: {text:?}"
        );
    }
